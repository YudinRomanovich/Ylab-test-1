...
class CacheRepository:
    ...
    async def set_all_submenus_cache(self, menu_id: str, submenu_data: list[Submenu]) -> None:
        await self.cacher.set(
    ...
    async def get_all_submenus_cache(self, menu_id: str) -> list[Submenu] | None:
    ...
    async def set_submenu_cache(self, menu_id: str, submenu_data: Submenu) -> None:
        await self.cacher.set(
            SUBMENU_URL.format(menu_id=menu_id, submenu_id=str(submenu_data.id)),
            pickle.dumps(submenu_data),
            ex=EXPIRE,
        )

    async def get_submenu_cache(self, menu_id: str, submenu_id: str) -> Submenu | None:
        cache = await self.cacher.get(SUBMENU_URL.format(menu_id=menu_id, submenu_id=submenu_id))
        if cache:
            submenu_data = pickle.loads(cache)
            return submenu_data
        return None
    ...
    async def update_submenu_cache(self, menu_id: str, submenu_data: Submenu) -> None:

        await self.delete_all_submenu_cache(str(submenu_data.menu_id))
        await self.set_submenu_cache(menu_id=menu_id, submenu_data=submenu_data)


    async def delete_all_submenu_cache(self, menu_id: str) -> None:
        await self.cacher.delete(SUBMENUS_URL.format(menu_id=menu_id))

    async def delete_submenu_cache(self, menu_id: str, submenu_id: str) -> None:
        await self.delete_cache_by_mask(
